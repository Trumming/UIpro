						   ...
						   UINT  writebuffersize)	// size to the writebuffer
{
	...
	//char *szPort = new char[50];//这里修改了原来的数据类型，以适用于UNICODE编码
	...
	//sprintf(szPort, _T("COM%d"), portnr);//这里修改了原来的数据处理方式，以适用于UNICODE编码
	//sprintf(szBaud, _T("baud=%d parity=%c data=%d stop=%d"), baud, parity, databits, stopbits);

	swprintf((wchar_t *)szPort, _T("COM%d"), portnr);//另外一种处理方式
	swprintf((wchar_t *)szBaud, _T("baud=%d parity=%c data=%d stop=%d"), baud, parity, databits, stopbits);
...
void CSerialPort::ProcessErrorMessage(TCHAR* ErrorText)
{
	...
	swprintf(Temp, _T("WARNING:  %s Failed with the following error: \n%s\nPort: %d\n"), (TCHAR*)ErrorText, lpMsgBuf, m_nPortNr); 
